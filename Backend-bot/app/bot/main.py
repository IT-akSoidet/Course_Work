...
from app.integrations.public_schedule_sync import sync_public_schedule
...
BOT_COMMANDS = [
    ...
    BotCommand(command="sync_schedule", description="Синхронизация расписания"),
    BotCommand(command="sync_schedule_autofill", description="Автодобавление аудиторий + синк"),
]
...
async def run_bot() -> None:
    ...
    try:
        if (
            settings.schedule_sync_interval_hours > 0
            and (settings.schedule_index_url or settings.schedule_sheet_urls)
        ):
            schedule_task = asyncio.create_task(_run_periodic_schedule_sync(settings))
            logger.info(
                "Periodic schedule sync enabled every %s hours.",
...
async def _run_periodic_schedule_sync(settings) -> None:
    while True:
        try:
            async with SessionLocal() as session:
                result = await sync_public_schedule(
                    session,
                    index_url=settings.schedule_index_url,
                    sheet_urls=settings.schedule_sheet_urls,
                    source="hse_public_sheets",
                    replace_source=True,
                )
            logger.info(
                "Schedule sync done: sheets=%s imported=%s skipped_unknown=%s skipped_online=%s",
                result.sheet_urls_total,
                result.imported_slots,
                result.rows_skipped_unknown_room,
                result.rows_skipped_online_or_empty,
            )
        except Exception:
            logger.exception("Periodic schedule sync failed.")

        interval = max(1, settings.schedule_sync_interval_hours)
